...
```{python}
# create a variable called "groups_raw" from the url "https://drive.google.com/drive/u/1/folders/1gBsinvzPKDOfBhYwpyGRkqFY6q-LCaYi""
...
```{python}
# create a variable called "capitulos_raw" from the url "https://drive.google.com/file/d/1YhtuesiVtpOTtZP9p1noZ-eOfcLuc_3G/view?usp=share_link"
innovations_url = "https://drive.google.com/file/d/1g13GEnBqF7NhedB1DWeUSKdCPlH62-1i/view?usp=sharing"
innovations_url = 'https://drive.google.com/uc?id=' + innovations_url.split('/')[-2]
innovations_raw = pd.read_csv(innovations_url, sep=';')
del innovations_url
institution_innovations = preprocessing_groups_innovations(groups_raw, innovations_raw)
```

## Books

```{python}
# create a variable called "books_raw" from the url "https://drive.google.com/file/d/1JZyokRCDKCOptr_mVV-YE3NoyMJCRafN/view?usp=sharing"
books_url = "https://drive.google.com/file/d/1JZyokRCDKCOptr_mVV-YE3NoyMJCRafN/view?usp=sharing"
books_url = 'https://drive.google.com/uc?id=' + books_url.split('/')[-2]
books_raw = pd.read_csv(books_url)
del books_url
institution_books = preprocessing_groups_books(groups_raw, books_raw)
```
...
```{python}
# create a variable called "books_raw" from the url "https://drive.google.com/file/d/1JZyokRCDKCOptr_mVV-YE3NoyMJCRafN/view?usp=sharing"
softwares_url = "https://drive.google.com/file/d/16Yc3d65u8-bMWVMw1xSy0mZdi13TkZN-/view?usp=sharing"
softwares_url = 'https://drive.google.com/uc?id=' + softwares_url.split('/')[-2]
softwares_raw = pd.read_csv(softwares_url)
del softwares_url
institution_softwares = preprocessing_groups_softwares(groups_raw, softwares_raw)
```

## Thesis

```{python}
thesis_url = "https://drive.google.com/file/d/1wdjYQ4_NSLvXJEqzabUhFzzHlWBf43Zk/view?usp=sharing"
thesis_url = 'https://drive.google.com/uc?id=' + thesis_url.split('/')[-2]
output = "thesis.csv"
gdown.download(thesis_url, output, quiet=False)
thesis_raw = pd.read_csv('thesis.csv')
institution_thesis = preprocessing_groups_thesis(groups_raw, thesis_raw)
del thesis_url
del output 
```
...
institution_data = pd.merge(institution_groups, 
                            institution_papers, 
                            on='instituciones', 
                            how='outer')
institution_data = pd.merge(institution_data, 
                            institution_chapters, 
                            on='instituciones', 
                            how='outer')
institution_data = pd.merge(institution_data, 
                            institution_innovations, 
                            on='instituciones', 
                            how='outer')
institution_data = pd.merge(institution_data, 
                            institution_books, 
